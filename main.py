...
class VisionSensor(ValueHandler):
    ...
    def create_image_info_tile(self, tile_id):
        if self.image_tile[tile_id] is not None:
            ...
            cv2.line(self.image_info_tile[tile_id], (0, self.edge_position.value), (600, self.edge_position.value), (0, 0, 255), 2)
            cv2.line(self.image_info_tile[tile_id], (0, self.edge_position_tile[tile_id].value), (600, self.edge_position_tile[tile_id].value), (255, 0, 0), 2)
            if self.edge_is_in_position.value:
                cv2.line(self.image_info_tile[tile_id], (0, self.edge_position.value), (600, self.edge_position.value), (0, 255, 0), 2)
            else:
                cv2.line(self.image_info_tile[tile_id], (0, self.edge_position.value), (600, self.edge_position.value), (0, 255, 0), 1)

    def check_video(self):
        self.video_data = self.video_queue.tryGet()
        if self.video_data is not None:
            self.image_video = self.video_data.getData()
            self.image_video_base64 = base64.b64encode(self.image_video)
    ...
    # def create_live_view_thumbnail(self):
    #     np_thumbnail = jpeg.decode(self.image_video)
    ...
    while True:
        ...
        if vs_front.check_video():
            ...
            if vs_front_enable_live_view.value:
                mqtt.setMqttValue(mqtt.pTopics_vsController.set_vsFront_imageDataTn, vs_front.image_video_base64)

            if showOutput:
                inp_front = np.asarray(bytearray(vs_front.image_video), dtype=np.uint8)
                i0_front = cv2.imdecode(inp_front, cv2.IMREAD_COLOR)
                cv2.line(i0_front, (vs_front.edge_position.value, 0), (vs_front.edge_position.value, 800), (255, 0, 0), 2)
        ...
        if vs_rear.check_video():
            if vs_rear_enable_live_view.value:
                mqtt.setMqttValue(mqtt.pTopics_vsController.set_vsRear_imageDataTn, vs_rear.image_video_base64)

            if showOutput:
                inp_rear = np.asarray(bytearray(vs_rear.image_video), dtype=np.uint8)
                i0_rear = cv2.imdecode(inp_rear, cv2.IMREAD_COLOR)
                cv2.line(i0_rear, (preview_height_crop - vs_rear.edge_position.value, 0), (preview_height_crop - vs_rear.edge_position.value, 800), (255, 0, 0), 2)

                if i0_front is not None and i0_rear is not None:
                    i0_complete = np.concatenate((i0_rear, i0_front), axis=1)
